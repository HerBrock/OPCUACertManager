...
- Check if a valid CA exists for the project.
"""
...
CERT_LOG_HEADER = [
    ...
    "estado",  # "created", "updated", "skipped", "error", "deleted"
]
...
def has_valid_ca(project_folder: str | Path) -> bool:
    """
    Check if a project has a valid CA (both ca_key.pem and ca_cert.pem exist).

    Parameters:
        project_folder: Path to the project folder.

    Returns:
        True if CA files exist, False otherwise.
    """
    project_path = Path(project_folder)
    ca_folder = project_path / "certs" / "ca"
    
    ca_key = ca_folder / "ca_key.pem"
    ca_cert = ca_folder / "ca_cert.pem"
...
def log_certificate(
    ...
    Parameters:
        ...
        sujeto: Full subject string.
        emisor: Full issuer string.
        estado: Status of the operation ("created", "updated", "skipped", "error", "deleted").
    """
...
def export_log_to_csv(
    ...
    """
    Export the certificate log to a CSV file at the specified location.

    Parameters:
        project_folder: Path to the project folder.
        output_path: Path where the exported CSV will be saved.
    """
    log_data = get_certificate_log(project_folder)
    
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CERT_LOG_HEADER)
        writer.writeheader()
        writer.writerows(log_data)


def mark_certificate_as_deleted(
    project_folder: str | Path,
    cert_name: str,
) -> None:
    """
    Mark a certificate as deleted in the log (without removing the historical record).

    Parameters:
        project_folder: Path to the project folder.
        cert_name: Name of the certificate to mark as deleted.
    """
    log_path = get_project_log_path(project_folder)
    ...
    entries = get_certificate_log(project_folder)
    
    # Find the most recent entry for this certificate and mark it as deleted
    for entry in reversed(entries):
        if entry["nombre_certificado"] == cert_name:
            # Add a new entry marking it as deleted
            log_certificate(
                project_folder=project_folder,
                nombre_certificado=cert_name,
                tipo=entry["tipo"],
                ruta_completa=entry["ruta_completa"],
                fecha_expiracion=entry["fecha_expiracion"],
                sujeto=entry["sujeto"],
                emisor=entry["emisor"],
                estado="deleted",
            )
